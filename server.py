...
class BasicCard(Card):
    def __init__(self, card_id, name, description, card_def):
        ...
        self.is_exchangable = card_def['is_exchangable'] if "is_exchangable" in card_def else True
        self.is_playable = card_def['is_playable'] if "is_playable" in card_def else False
        self.is_discardable = card_def['is_discardable'] if "is_discardable" in card_def else True
        self.require_player_lock = card_def['require_player_lock'] if "require_player_lock" in card_def else False
        self.is_reactable = card_def['is_reactable'] if "is_reactable" in card_def else False
        self.neighbours_only = card_def['neighbours_only'] if "neighbours_only" in card_def else False
        self.name_displayed = card_def['name_displayed'] if "name_displayed" in card_def else ""
...
class Player:
    ...
    def to_dict(self, include_hand=False):
        data = {
            ...
            'hand': [{'name': c.name, 'name_displayed':c.name_displayed,'image':c.image, 'show_to':c.show_to} for c in self.hand],
        ...
        if include_hand:
            data['hand'] = [{'name': c.name, 'name_displayed':c.name_displayed,'image':c.image, 'show_to':c.show_to} for c in self.hand]
        return data
...
class CrossRelations():
    ...
    def add_relation_pl_pl(self, player_1, player_3, is_reflective = False):
        ...
        player_2_name = self.pl_pl_relations[player_1_name] if player_1_name in self.pl_pl_relations else ""
    ...
    def exchange_cards(self, player_1, player_2):
        ...
        player_1_cidx = self.pl_cd_relations[player_1_name]#self.get_selected_card(player_1)
        player_2_cidx = self.pl_cd_relations[player_2_name]#self.get_selected_card(player_2)
        card1, card2 = player_1.extract_selected_card(player_1_cidx), player_2.extract_selected_card(player_2_cidx)
    ...
    def to_dict(self):
        data = {
            'pl_pl': self.pl_pl_relations,
            'pl_cd': self.pl_cd_relations}
        return data
...
class Game:
    ...
    def is_reaction_valid(self, player, action_type):
        ...
        prev_player = self.get_next_player(self.current_player_index, -1 * self.direction)
        ...
        if self.cross.get_selected_card(player).require_player_lock and not target_player:
        ...
        if self.cross.get_selected_card(player).neighbours_only:
            if not target_player.nickname in [next_player.nickname, prev_player.nickname]:
    ...
    def is_action_valid(self, player, action_type):
        ...
        if action_type == "play":
            ...
            if self.cross.get_selected_card(player).require_player_lock and not target_player:
                logging.info(f"{player.nickname} cannot play card - no target is selected")
                return False

            if self.cross.get_selected_card(player).neighbours_only:
                if not target_player.nickname in [next_player.nickname, prev_player.nickname]:
                    self.end_turn_error(player, f"{player.nickname} cannot play card - neighbours only")
                    return False
        
        if action_type == "discard" and not self.cross.get_selected_card(player).is_discardable:
    ...
    def is_basic_exchange_possible(self, player, current_player, next_player):
        ...
        if selected_card.is_exchangable or (selected_card.name == "Infection" and player_1.is_thing) or (selected_card.name == "Infection" and player_1.is_infected and player_2.is_thing):
    ...
    async def process_action(self, player, action):
        ...
        if self.phase == 'draw':
            ...
            if action['action'] == 'play':
                ...
                if card.name == "Signal rocket":
                    ...
                    target_player = self.get_by_nickname(self.cross.get_target(player))
                    if current_player.could_exchange() and target_player.could_exchange():
                        current_player.preferred_target = target_player.nickname
                        self.set_exchange_phase()
                        return
                    logging.info(f"Player {current_player.nickname} cannot select exchange with {target_player.nickname}")
                    return
                    ...
                    self.post_action_stack = [card.name, target_player, target_index, index_1, index_2]
                ...
                if player.is_tranquilised>0 and card.name in ["Oil", "Necronomadd_relation_pl_plicon"]:
        ...
        elif self.phase == "post-action":            ########################################### POST_ACTION
            ...
            if action['action'] == 'react':
                ...
                if source_card_name in ["Flamethrower", "Sniper rifle"]:
                    if source_card_name == "Flamethrower" and current_card.name not in ["Necronomicon", "Armor"]:
                        self.end_turn_error(player, "Cannot dodge with current card, need necronomicon, armor")
                        return
                    elif source_card_name == "Sniper rifle" and current_card.name not in ["Necronomicon", "Armor", "Oil"]:
                        self.end_turn_error(player, "Cannot dodge with current card, need necronomicon, armor, oil")
                        return
                    logging.info("Player saved from flamethrower")
                elif source_card_name == "Blood Sample":
                    if current_card.name not in ["Necronomicon"]:
                        self.end_turn_error(player, "Cannot dodge with current card, need necronomicon")
                        return
                    ...
                    if current_card.name not in ["Necronomicon", "Fuck You"]:
                        self.end_turn_error(player, "Cannot dodge with current card, need necronomicon, fuck you")
                        return
                    logging.info("Player saved from changing places")
                else:
                    self.end_turn_error(player,f"Unkown source for reaction {source_card_name}")
                    return
                ...
                if source_card_name == "Flamethrower":
                    ...
                    self.players[self.post_action_stack[3]], self.players[self.post_action_stack[4]] = self.players[self.post_action_stack[4]], self.players[self.post_action_stack[3]]
                    ...
                    # self.post_action_stack = [card.name, current_player.nickname, target_player]
                    self.post_action_stack[2].hand[random.randint(0,len(self.post_action_stack[2].hand)-1)].show_to.append(self.post_action_stack[1])
                else:
                    self.end_turn_error(player, f"Unkown source for confirm {source_card_name}")
                    return
                ...
                action_type = action['action']
                self.end_turn_error(player, f"Unknown reaction event {action[action_type]}")
                return
            ...
            if action['action'] == 'react':
                ...
                if not card.name in ["Fear", "Talk Out", "Necronomicon"]:
                    ...
                    self.end_turn_error(player, f"Card {card.name} could be dodged only with fear, talk out, necronomicon")
                    return
                ...
                if card.name == "Talk Out":
                    ...
                    if player.nickname in [next_player.nickname, prev_player.nickname]:
                        ...
                        paired_player = next_player if player == current_player else current_player
                ...
                if not next_player.could_exchange() or not current_player.could_exchange():
                    ...
                    if not player.nickname in [current_player.nickname, next_player.nickname]:
                        return

                    if not self.is_basic_exchange_possible(player, current_player, next_player):
                    ...
                    current_card, next_card = self.cross.get_selected_card(current_player), self.cross.get_selected_card(next_player)
        ...
        if is_turn_ended:
            self.current_player_index = self.get_next_player_index(self.current_player_index, self.direction)
            self.phase = 'draw'
            self.end_turn_clean()
            self.post_action_stack = []
...
async def broadcast_game_state(game):
    ...
    for player in game.players:
        ...
        if player.is_active and hasattr(player, 'ws') and \
           isinstance(player.ws, web.WebSocketResponse) and \
           not player.ws.closed:
            try:
    ...
    async for msg in ws:
        if msg.type == aiohttp.WSMsgType.TEXT:
            ...
            if data['type'] == 'login':
                ...
                existing = next((p for p in game.players if p.nickname == nickname), None)
                if existing:
                ...
                if len(game.players) == game.config['players_per_game'] and game.phase == 'waiting':
